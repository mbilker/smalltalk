...
(function () {
  ...
  function SmallTalk(callback) {
    ...
    this.dropdown = function (title, msg, value, options) {
      var val = value || {};
      var valueStr = '<select data-name="js-dropdown">';
    ...
    function showDialog(title, msg, value, buttons, options) {
      ...
      find(dialog, ['ok', 'input', 'dropdown']).forEach(function (el) {
        el.focus();
      });
    ...
    function keyDown(dialog, ok, cancel) {
      return function (event) {
        ...
        var namesAll = ['ok', 'cancel', 'input', 'dropdown'];
    ...
    function changeButtonFocus(dialog, names) {
      ...
      if (activeName !== 'input' && activeName !== 'dropdown' && count && isButton) {
    ...
    function closeDialog(el, dialog, ok, cancel) {
      ...
      if (/close|cancel/.test(name)) {
        ...
        var value = find(dialog, ['input', 'dropdown']).reduce(function (value, el) {
          return el.value;
        }, null);
